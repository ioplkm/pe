...
void resolvePointCollision(PointCollision *pCollision, double dTime) {
  resolvePointVelocity(pCollision->pPoint1, pCollision->pPoint2, pCollision->normal, pCollision->e, dTime);
  resolvePointInterpenetration(pCollision->pPoint1, pCollision->pPoint2, pCollision->normal, pCollision->penetration);
}

/*void resolvePointCollisions(double dTime) {
  for (int i = 0; i < collisionsAmount; i++) {
    resolvePointCollision(&pointCollisions[i], dTime);
  }
}*/
